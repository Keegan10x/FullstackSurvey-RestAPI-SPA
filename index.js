...
1) identify the correct datatype (JSON), the head, is authorised
2) check if is file
...
app.use(async (context, next) => {
  ...
  //!(context.request.url.pathname.includes("/api/v1/surveys") && context.request.method === 'GET')
  //!(context.request.url.pathname.includes("/api/v1/accounts") && context.request.method === 'POST')
  if (context.request.url.pathname.includes("/api/v1") && !(context.request.url.pathname.includes("/api/v1/surveys") && context.request.method === 'GET') && !(context.request.url.pathname.includes("/api/v1/accounts") && context.request.method === 'POST')) {
    ...
    if (
      context.request.headers.get("Content-Type") !== "application/vnd.api+json"
    ) {
          ...
          errors: [
            {
              ...
              detail:
                "This API supports the JSON:API specification, Content-Type must be application/vnd.api+json",
            },
          ],
        },
        null,
          ...
          errors: [
            {
              ...
              detail:
                "the API uses HTTP Basic Auth and requires a correctly-formatted Authorization header",
            },
          ],
        },
        null,
...
// checks if file exists NOT A MIDDLE WARE FUNCTION
